package com.vansz.extpagerlayoutmanager.core;
...
public abstract class ViewPagerLayoutManager extends LinearLayoutManager {
    ...
    private boolean mFirstAlignTop = false;
    ...
    public ViewPagerLayoutManager(Context context) {
        this(context, HORIZONTAL, false, false);
    }
     ...
     */
    public ViewPagerLayoutManager(Context context, int orientation, boolean reverseLayout, boolean firstTopAlign) {
        super(context);
        setOrientation(orientation);
        setReverseLayout(reverseLayout);
        setAutoMeasureEnabled(true);
        setItemPrefetchEnabled(false);
        mFirstAlignTop = firstTopAlign;
    }
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        mSpaceMain = mFirstAlignTop ? 0 : (mOrientationHelper.getTotalSpace() - mDecoratedMeasurement) / 2;
    ...
    private int scrollBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        mOffset += realDx;
        // recycle item
        recycleByScrolling(dy, recycler);
        //handle recycle
        layoutItems(recycler);
    ...
    /**
     * Recycling the item outside the screen while scrolling
     */
    private void recycleByScrolling(int dy, RecyclerView.Recycler recycler) {
        updateScrollingOffsetState(dy);
        if (mScrollingOffset != Integer.MAX_VALUE) {
            if (mAvailable < 0) {
                mScrollingOffset += mAvailable;
            }
            recycleByLayoutState(recycler, dy);
        }
    }

    private void updateScrollingOffsetState(int dy) {
        if (dy > 0) {
            final View child = getChildClosestToEnd();
            mScrollingOffset = mOrientationHelper.getDecoratedEnd(child)
                    - mOrientationHelper.getEndAfterPadding();

        } else {
            final View child = getChildClosestToStart();
            mScrollingOffset = -mOrientationHelper.getDecoratedStart(child)
                    + mOrientationHelper.getStartAfterPadding();
        }
        mAvailable = Math.abs(dy);
        mAvailable -= mScrollingOffset;
    }
     ...
     */
    private void recycleByLayoutState(RecyclerView.Recycler recycler, int dy) {
        if (mInfinite) return;

        if (dy < 0) {
            recycleViewsFromEnd(recycler, mScrollingOffset);
        } else {
            recycleViewsFromStart(recycler, mScrollingOffset);
        }
    }
    ...
    /**
     * used by SnapHelper to center the current view
     *
